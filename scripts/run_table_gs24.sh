#!/usr/bin/env bash
# Gaussian-splat-on-mesh training for data/table, run 24.
#
...
#
# ---------------------------------------------------------------------------------------
# 3. coverage_rescue_thresh 1.0 -> 0.5, coverage_rescue_max_frac 0.05 -> 0.02
#
# gs23 ran without a ceiling and died of CUDA OOM at step 6001 with 8,021,019 rows, having
# started the run at 4.88M. The growth was almost entirely coverage_rescue:
#
#     step  990:  290,336 faces qualified -> 244,012 spawned  (capped at max_frac 5%)
#     step 5990:  104,416 faces qualified -> 104,416 spawned  (all of them)
#     Splitting over the same window: 95 rows. Densification was not the cause.
#
# coverage_rescue_thresh 1.0 is the HIGHEST setting: every face whose centroid or corner
# coverage density falls below 1.0 counts as needing repair. analyze_splat_ply measured the
# face-centroid distribution on gs20 at q1/q5/q25/q50 = 0.008/0.234/0.950/1.029 -- the
# median sits right on 1.0, so roughly half the mesh is permanently 'qualified'. Meanwhile
# the real gap coverage (union test, alpha >= 0.01) has measured 99.99% every time. rescue
# was refilling holes that are not holes.
#
# It compounds because rescue spawns do not survive -- gs15 established that: a cycle
# spawns hundreds of thousands and culling removes almost all of them, since a Gaussian
# born late into a converged solution cannot earn opacity. So the deficit reappears next
# cycle, rescue fires again, and the population ratchets up 5% each time.
#
# The value is a leftover, not a decision. It was 0.2 before gs8, 0.9 at gs8, and gs15
# raised it to 1.0 to test whether rescue was a detail producer -- that hypothesis was
# refuted in the same run, but the threshold stayed at 1.0 for gs11 through gs23. It has
# never been examined as a population control.
#
# 0.5 sits below gs20's q25, so only genuinely sparse faces trigger. max_frac 0.02 caps the
# per-cycle growth at 2% as a second line of defence. Watch the run's own final coverage
# line: if it drops below ~99.9%, raise the threshold back towards 0.7 rather than
# reinstating a headcount ceiling.
#
# ---------------------------------------------------------------------------------------
...
#   bash scripts/run_table_gs24.sh
#   bash scripts/run_table_gs24.sh --export-only
...
# -- RESUME: pick up from the newest checkpoint if this run was interrupted ---------
# steps-per-save is 10000 and save_only_latest_checkpoint is on, so at most one file is
# here and it is the newest. Without --load-dir nerfstudio silently starts from scratch,
# which on a 4-hour run is an expensive way to find out.
...
if [[ -d "${CKPT_DIR}" ]] && compgen -G "${CKPT_DIR}/step-*.ckpt" > /dev/null; then
    ...
    echo "[pre-flight] no checkpoint, training from scratch"
fi
...
PYTHONPATH="${SPLAT_DIR}" python nerfstudio/scripts/train.py splatfacto_on_mesh_uc \
    ...
    --pipeline.model.coverage_rescue_thresh 0.5 `# gs11-gs23: 1.0 -- see section 3` \
    --pipeline.model.coverage_rescue_max_frac 0.02 `# gs11-gs23: 0.05` \
